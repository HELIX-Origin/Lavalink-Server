...
| :--- | :--- | :--- | :--- |
| **Server Variable** | `PORT`, `LAVA_PASS`, `YOUTUBE_CIPHER_URL`, `SPOTIFY_CLIENT_ID` | Lavalink Server (`application.yml`, `.env`, `docker-compose.yml`) | Tells Lavalink what port to bind to, what password to demand, and which external APIs to query. |
...
| `YOUTUBE_REFRESH_TOKEN` | *(empty)* | YouTube OAuth 2.0 refresh token generated by `/youtube-auth`. Once a device flow completes the token is printed to the console and persisted automatically. |
...
| `YOUTUBE_SKIP_INIT` | `true` | When `true` the server-run flow (using `YOUTUBE_API_KEY`) issues the authorization URL instead of the in-Java plugin flow. |
